...
import { responses, simpleResponse } from '@/router/globalMiddlewares/response.middlewares';
...
router.route('/filter')
  .get(
    (req, res, next) => handler(getValidator, req, res, next, validations['getWithFilter']),
    (req, res, next) => simpleResponse(deliveriesCommonServices.findWithFilters, req, res, next))
